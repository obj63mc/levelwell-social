...
Everything needed on the Meta side (developers.facebook.com) before LevelWell Social can post. Facts verified against Meta docs as of Aug 2026 (Graph API **v25.0**; pin in one constant, plan an annual version bump — Meta ships ~3 versions/year, each supported ~2 years).

## 1. Account & Profile Prerequisites

- [ ] A Facebook account with a **Meta developer account** (register at [developers.facebook.com](https://developers.facebook.com) — instant, free).
- [ ] For each profile the app will manage:
  - [ ] The Instagram account is converted to **Professional** (Business or Creator): Instagram app → Settings → Account type.
  - [ ] The IG professional account is **linked to a Facebook Page** you admin (Page Settings → Linked accounts → Instagram). This link is what makes the IG account reachable through the Facebook-Login token chain.
  - [ ] You are an **admin** of the Facebook Page (full control).

## 2. Create the App

- [ ] developers.facebook.com → My Apps → **Create App** (use-case-based flow):
...
- [ ] App Settings → Basic: record **App ID** and **App Secret** → store as Convex env vars (`META_APP_ID`, `META_APP_SECRET`; see [`CONVEX.md`](./CONVEX.md) §2). The secret lives only in Convex.
- [ ] Do **not** set the app category to Native/Desktop in Advanced settings — token exchange happens server-side in Convex, which is exactly Meta's recommended pattern, and Native/Desktop mode disables secret-based server calls.
...
- [ ] Products → Facebook Login for Business → Configurations → **Create configuration**.
...
- [ ] Record the **`config_id`** → Convex env var `META_LOGIN_CONFIG_ID`. The authorize URL uses `config_id` (not `scope`).

## 4. OAuth Redirect Configuration

- [ ] Facebook Login for Business → Settings → **Valid OAuth Redirect URIs**: add both Convex deployments:
  - `https://<dev-deployment>.convex.site/oauth/callback`
  - `https://<prod-deployment>.convex.site/oauth/callback`
- [ ] Flow recap: desktop app opens the system browser → `https://www.facebook.com/v25.0/dialog/oauth?client_id=…&config_id=…&state=…&redirect_uri=…` → Meta redirects to the Convex HTTP action → callback verifies `state`, exchanges `code` for a short-lived user token, upgrades it via `grant_type=fb_exchange_token` (long-lived, ~60 days), calls `GET /me/accounts` to derive **Page access tokens (no expiration)**, resolves the linked IG account via `GET /{page-id}?fields=instagram_business_account`, stores all of it, and renders a "connected — return to the app" page.
- [ ] Smoke-test early: `.convex.site` is a standard HTTPS domain and should be accepted, but verify the redirect round-trip before building on it.

## 5. Webhooks (comments inbox)

- [ ] Products → **Webhooks** → add product.
- [ ] Callback URL: `https://<deployment>.convex.site/webhooks/meta`; Verify token: the value of `META_WEBHOOK_VERIFY_TOKEN`. Meta sends a GET with `hub.challenge` — the endpoint must echo it (implement before clicking Verify).
- [ ] Subscribe fields:
  - **Instagram** topic → `comments` field.
  - **Page** topic → `feed` field (covers comments on Page posts).
- [ ] Per connected Page, the backend must also call `POST /{page-id}/subscribed_apps?subscribed_fields=feed` (requires `pages_manage_metadata`) — wire into the profile-connect flow.
- [ ] All webhook POSTs must be validated: `X-Hub-Signature-256` HMAC-SHA256 over the raw body using the App Secret.
...
- [ ] IG account(s) Professional + linked to admin'd Page(s)
- [ ] Business-type app created with Pages + Instagram use cases
- [ ] App ID / App Secret stored in Convex env vars
- [ ] Facebook Login for Business configuration created; `config_id` stored
- [ ] OAuth redirect URIs (dev + prod `.convex.site`) registered and round-trip tested
